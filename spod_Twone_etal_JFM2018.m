function [L,P,f,Lc,Q,TH,Lreal] = spod(X,varargin)
...
if ~opts.savefft
    ...
    for iFreq = 1:nFreq
        ...
        [Theta(:,:,iFreq),Lambda]      = eig(M);
        Lambda              = diag(Lambda);
        [Lambda,idx]        = sort(Lambda,'descend');
        Theta(:,:,iFreq)    = Theta(:,idx,iFreq);
        Psi                 = Q_hat_f*Theta(:,:,iFreq)*diag(1./sqrt(Lambda)/sqrt(nBlks));
...
% disp(['Windowing value arnau     : ' num2str(window(3581))])
